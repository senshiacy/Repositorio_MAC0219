...
void* compute_mandelbrot(void* id){
    struct linhas* linhas_id = (struct linhas*) id;
    int linha_inicial_id = linhas_id->linha_inicial;
    int linha_final_id = linhas_id->linha_final;
    ...
    for(i_y = linha_inicial_id; i_y < linha_final_id; i_y++){
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    pthread_t threads[N];
    int linhas_por_thread = i_y_max/N;

	
    for (int i = 0; i < N; i++) {
        struct linhas* intervalo = malloc(sizeof(struct linhas));
        intervalo->linha_inicial = i*linhas_por_thread;
        if (i < N-1) {
            intervalo->linha_final = (i+1)*linhas_por_thread;
        } else {
            intervalo->linha_final = i_y_max;
        }

        pthread_create(&threads[i], NULL, compute_mandelbrot, intervalo);
    }
